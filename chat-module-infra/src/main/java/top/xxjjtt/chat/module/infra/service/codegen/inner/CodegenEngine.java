package top.xxjjtt.chat.module.infra.service.codegen.inner;
...
import top.xxjjtt.chat.framework.apilog.core.annotation.ApiAccessLog;
import top.xxjjtt.chat.framework.apilog.core.enums.OperateTypeEnum;
...
import top.xxjjtt.chat.framework.common.util.collection.CollectionUtils;
import top.xxjjtt.chat.framework.common.util.date.DateUtils;
import top.xxjjtt.chat.framework.common.util.date.LocalDateTimeUtils;
import top.xxjjtt.chat.framework.common.util.object.BeanUtils;
import top.xxjjtt.chat.framework.common.util.object.ObjectUtils;
import top.xxjjtt.chat.framework.common.util.string.StrUtils;
import top.xxjjtt.chat.framework.excel.core.annotations.DictFormat;
import top.xxjjtt.chat.framework.excel.core.convert.DictConvert;
import top.xxjjtt.chat.framework.excel.core.util.ExcelUtils;
...
import top.xxjjtt.chat.module.infra.dal.dataobject.codegen.CodegenColumnDO;
import top.xxjjtt.chat.module.infra.dal.dataobject.codegen.CodegenTableDO;
import top.xxjjtt.chat.module.infra.enums.codegen.CodegenFrontTypeEnum;
import top.xxjjtt.chat.module.infra.enums.codegen.CodegenSceneEnum;
import top.xxjjtt.chat.module.infra.enums.codegen.CodegenTemplateTypeEnum;
import top.xxjjtt.chat.module.infra.enums.codegen.CodegenVOTypeEnum;
import top.xxjjtt.chat.module.infra.framework.codegen.config.CodegenProperties;
...
public class CodegenEngine {
    ...
    /**
     * 是否为 chat-cloud 项目，用于解决 Boot 和 Cloud 的 api 模块兼容性问题
     *
     * true  - 需要有 chat-module-xxx-api 模块
    ...
    public CodegenEngine() {
        ...
        this.cloudEnable = ClassUtils.isPresent("top.xxjjtt.chat.module.infra.framework.rpc.config.RpcConfiguration",
    ...
    private Map<String, String> getTemplates(Integer frontType) {
        ...
        if (Boolean.FALSE.equals(cloudEnable)) {
            SERVER_TEMPLATES.forEach((templatePath, filePath) -> {
                filePath = StrUtil.replace(filePath, "/chat-module-${table.moduleName}-api", "");
                filePath = StrUtil.replace(filePath, "/chat-module-${table.moduleName}-server", "");
    ...
    private static String javaModuleFilePath(String path, String module, String src) {
        return "chat-module-${table.moduleName}/" + // 顶级模块
                "chat-module-${table.moduleName}-" + module + "/" + // 子模块
                "src/" + src + "/java/${basePackage}/module/${table.moduleName}/" + path + ".java";
    }

    private static String mapperXmlFilePath() {
        return "chat-module-${table.moduleName}/" + // 顶级模块
                "chat-module-${table.moduleName}-server/" + // 子模块
    ...
    private static String vueFilePath(String path) {
        return "chat-ui-${sceneEnum.basePackage}-vue2/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3FilePath(String path) {
        return "chat-ui-${sceneEnum.basePackage}-vue3/" + // 顶级目录
                "src/" + path;
    }

    private static String vue3VbenFilePath(String path) {
        return "chat-ui-${sceneEnum.basePackage}-vben/" + // 顶级目录
                "src/" + path;
    }
